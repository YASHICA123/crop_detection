...
plant_health_df = pd.read_csv(os.path.join(BASE_DIR, 'Crop_Health_Tips.csv'))

# Load the CNN model only when TensorFlow is available
soil_cnn_model = None
soil_model_error = None
if tf is not None:
    try:
        soil_cnn_model = tf.keras.models.load_model(os.path.join(BASE_DIR, 'soil_cnn_model.h5'))
    except Exception as e:
        soil_model_error = str(e)
else:
    soil_model_error = "TensorFlow is not installed in the current environment."

# Load the trained RandomForestClassifier model
with open(os.path.join(BASE_DIR, 'model.pickle'), 'rb') as f:
...
def predict_soil():
    try:
        if soil_cnn_model is None:
            return jsonify({'error': f'Soil prediction unavailable: {soil_model_error}'}), 503
...
def transform_image(image):
    if img_to_array is None:
        raise RuntimeError("TensorFlow image preprocessing is unavailable.")
...
def classify_soil_type(image_file):
    ...
    try:
        if soil_cnn_model is None:
            raise RuntimeError(f"Soil prediction unavailable: {soil_model_error}")
